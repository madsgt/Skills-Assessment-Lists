...
def long_words(words):
    ...
    for char in words:
        if len(char) > 4:
            longerthan_4char.append(char)
        
    return longerthan_4char
...
def n_long_words(words, n):
    ...
    for char in words:
        if len(char) > n:
            longerthan_nchar.append(char)
        
    return longerthan_nchar
...
def halvesies(numbers):
    ...
    halved_numbers = []
    for i in numbers:  
        halved_numbers.append(float(i)/2) 
...
def average(numbers):
    ...
    """
    if numbers == []:
        avg_ofnumbers = 'undefined'
    else:
        total_ofnumbers = 0
        for i in numbers:
            total_ofnumbers += i

        avg_ofnumbers = float(total_ofnumbers)/len(numbers)

    return avg_ofnumbers
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    len_oflist = len(items)/2

    for i in range(len_oflist):
        x = items[i]
        items[i] = items[len(items)-i-1]
        items[len(items)-i-1] = x
...
def duplicates(items):
    ...
    for i in range(len(items)):
        for j in range(i+1, len(items)):
            if items[i] == items[j] and not items[i] in duplicate_words:
                duplicate_words.append(items[i])
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for i in words:
        ...
        for j in range(len(i)):
            if letter == i[j] and count < 1:
                count += 1
                indices_ofletters.append(j)

                
        if count == 0:
            indices_ofletters.append(None)

            

    return indices_ofletters
